...
select (total_points/sum(total_points))*100 as percentage_of_wins,bid_status,bd.bidder_id from ipl_bidding_details bd
...
select sum(total_matches) as total_number_of_matches,s.stadium_id,stadium_name,city from ipl_stadium s join ipl_match_schedule ms
on s.stadium_id=ms.stadium_id
join ipl_tournament t
on t.tournmt_id=ms.tournmt_id
group by stadium_name;

-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select toss_winner,match_winner,(sum(match_winner)/sum(toss_winner))*100 as percentage_of_wins, s.stadium_id,stadium_name,city from ipl_stadium s join ipl_match_schedule ms
on s.stadium_id=ms.stadium_id
join ipl_match m
...
select sum(no_of_bids) as total_number_of_bids,bid_team,team_name from ipl_bidder_points bs
join ipl_bidding_details bd on bs.bidder_id=bd.bidder_id
join ipl_team tm
on tm.team_id=bd.bid_team
group by team_name;

-- 5.	Show the team id who won the match as per the win details.
select team_id,matches_won from ipl_team_standings ;

-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select  sum(matches_played) total_matches,sum(matches_won) total_matches_won,sum(matches_lost) total_matches_lost,team_name,t.team_id from ipl_team_standings ts
join ipl_team t 
...
select count(player_role),team_id from ipl_team_players group by team_id;
select player_name,count(team_name) total_number_of_all_rounders_in_each_team,player_role ,team_name from ipl_player p
join ipl_team_players tp on p.player_id=tp.player_id
join ipl_team t
on t.team_id=tp.team_id
where player_role='all-rounder'
group by team_name
having count(team_name)>4;
